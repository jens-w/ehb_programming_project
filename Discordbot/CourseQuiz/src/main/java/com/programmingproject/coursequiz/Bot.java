...
public class Bot {
    public static void main(String[] args){
        ...
        client.getEventDispatcher().on(MessageCreateEvent.class).subscribe(event -> {
                    if ( event.getMessage().getAuthor().map(user -> !user.isBot()).orElse(false))//checks if not a bot
                    {
                        ...
                        //if(command.equals("!registreer")){user.registerUser(event, user, commandSplit, snowflake);}
                        if(command.startsWith("!registreer")){user.registerUser(event, user, commandSplit, snowflake);}
                    }
                });
    ...
    public static void executeCommand(MessageCreateEvent event, User user, String command, String[] commandSplit, String snowflake){
        ...
        else{botSendMessage(event, "Please register using your user key.");}//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<xxxxxxxxx
    }
